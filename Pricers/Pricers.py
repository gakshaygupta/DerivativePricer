...
class AmericanMC:
    ...
    def __init__(self, Num_Basis, Regressor, Derivative, IRModel):
        ...
        # This variable defines the number of basis that should be used for regression
        self.Num_Basis = Num_Basis
        # this variable defines which regression to use ( currently the regression is linear)
        self.Regressor = Regressor
        self.Derivative = Derivative  # This the derivative we want to price
        # this index tells us which indexes to be used for calculating the value
        self.PayOffIndex = Derivative.PayoffIndex
        # this is interest rate model . this can be stochastic or deterministic
        self.IRModel = IRModel
        # this variabl carries the riskfactor paths
        self.RF_Paths = Derivative.RiskFactorPaths()
        self.NumPaths, self.Maturity = self.Derivative.NumPaths, self.Derivative.Maturity
        # this discount curve will be between two corssecgtion
        self.DiscountCurve = IRModel.GetDiscountCurve(
            self.NumPaths, self.Maturity)
        # this matrix defines the stoping rule for pricing
        self.StoppingRule = [self.Maturity-2]*self.NumPaths
        # this matrix gives the payoff for each crossection
        self.PayOffMatrix = np.zeros_like(self.StoppingRule)
    ...
    def GenerateNPV(self):
        ...
        for index in self.PayOffIndex:
            ...
            Implied_Value.append(
                self.Derivative.GetImpliedPayOff(CorssSection))
        ...
        for index in range(len(self.PayOffIndex)-1, -1, -1):
            ...
            # this will be only for payoff index
            Discounted_IV[:, index:] = DF*Discounted_IV[:, index:]
